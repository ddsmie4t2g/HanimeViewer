 ...
 *
 * ⚠️ 26.9.4 起**一页就是站点自己的一页**（应用页号 = 站点页号）：站点一页给多少条就是多少条，
 * 不再有「应用内一页 12 条」的换算。所以 [videos] 的条数是 40–59 这种量级，末页不满很正常。
 *
 * @param siteTotalPages **站点那边这份列表一共有几页**（站点自己的分页口径）。
 *
 *   为什么需要它：没有它，分页条就只能「翻一页长一页」，用户看不到「一共多少页」。
 *   三条路实测：hanime 的搜索页是 Laravel 分页，页码条里直接写着末页号；
 *   nJAV 的女优页按站点自己的页号请求，**页面上有页码条就读出来**（2026-09-15 复核：
 *   女优一览 `/cn/actresses` 的页码条确凿存在 —— `rel=next` + `1..10` 数字链接；
 *   女优页本身在非浏览器客户端上频繁被反爬挑战页挡住，所以「读不到」是常态而非异常）；
 *   Pornhub 作者页的 `div.pagination3` 同页混着别的列表的页码（实测 p2 里能出现 18 / 25，
 *   取 max 会串页），所以 Pornhub **不填**这一项，改由「87 Videos ÷ 实测站点一页条数」换算。
 *
 *   拿不到就留 null。**这是站点页数**，与界面上的页号一一对应（26.9.4 起不再换算）。
 *
 * @param hasNext 站点**自己说**的这一页之后还有没有下一页。
 *
 *   `null` = 站点没给线索（调用方退回「按已知总数判断」）。
 *   Pornhub 的作者页给 `<link rel=next>`、nJAV 给 `<a rel=next>` —— 有就是 `true`，
 *   没有就是 `false`（**这是「下一页」按钮最可靠的闸门**，比估算的总页数可靠得多：
 *   估算偏小时用户会以为已经到底了，正是用户报的「最后的作品翻不到」）。
 */
