 ...
 * 首页右上角那个「功能入口」按钮的形态。
 *
 * 这三个数据源的可用页面本来就不同，所以它不是装饰，而是**按站点切换的导航**：
 *
 * | 数据源 | 按钮 | 去向 |
 * |---|---|---|
 * | hanime | 📅 日历 / 新番预告 | [PreviewRoute]（站方预告 + 按上市月归档 + Getchu 预告） |
 * | nJAV | 👥 浏览（女优一览 / 女优排行） | [ActressGalleryRoute] |
 * | Pornhub | **不显示** | —— |
 *
 * 为什么 Pornhub 干脆不给：日历页要读的是 hanime 的归档与 Getchu 的预告，
 * Pornhub 下点进去只有一整页空态（`getHanimeArchiveByMonth` 对 AV 站直接返回 NoMoreData），
 * 摆在那里只会让人以为坏了。
 */
